...
export const OPT_OUT_PENDING_ROUTE = new Set<string>([
    ...
    'BulkSendItem',
    // Same shape as ProgramSlackAuth above, newly surfaced by the personId fix
    // (its `personId` FK now makes it isScopable()). Reconcile-job-internal only,
    // no route yet — GET /api/settings/sync-status (spec §7.4/§12 PR3) is when
    // this earns a real SCOPE_BINDINGS row.
    'SyncState',
]);
