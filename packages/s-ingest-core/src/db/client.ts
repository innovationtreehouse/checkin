 ...
 *
 * Held at module scope so a warm Lambda invocation reuses the same client (and
 * its connection) instead of opening a new pool each call. In production the
 * SHOPIFY_READ_DATABASE_URL should route through RDS Proxy / pgBouncer with
 ...
 * both are governed by SHOPIFY_READ_DATABASE_URL params.
 *
 ...
 * guarded against drift by `test/unit/client.construction.test.ts`.
 */
 ...
 * SHOPIFY_READ_DATABASE_URL is read. Memoized on `globalThis`, so the first caller wins.
 */
export function getPrisma(
  connectionString: string | undefined = process.env.SHOPIFY_READ_DATABASE_URL,
): PrismaClient {
  if (globalThis.__shopifyReadPrisma) return globalThis.__shopifyReadPrisma;
  if (!connectionString) {
    throw new Error(
      "SHOPIFY_READ_DATABASE_URL is not set (and no connection string was passed to getPrisma()); " +
        "the Prisma client cannot be constructed without a database URL.",
    );
  }
  const adapter = new PrismaPg({ connectionString });
  const client = new PrismaClient({ adapter });
  globalThis.__shopifyReadPrisma = client;
  return client;
}

/**
 * Lazy singleton: every property access resolves through {@link getPrisma}, so the client
 * is built on first real use rather than at import. Existing `prisma.shopOrder.…` call sites
