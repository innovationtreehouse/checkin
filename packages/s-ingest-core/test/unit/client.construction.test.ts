 ...
 * `getPrisma()` env-read path — so without this test, renaming SHOPIFY_READ_DATABASE_URL or
...
const ENV = "SHOPIFY_READ_DATABASE_URL";
const CLIENT = "../../src/db/client.js";
let saved: string | undefined;

beforeEach(() => {
  saved = process.env[ENV];
  adapterArgs.length = 0;
  delete (globalThis as Record<string, unknown>).__shopifyReadPrisma;
  vi.resetModules();
});
...
describe("getPrisma construction (env-read wiring)", () => {
  it("reads SHOPIFY_READ_DATABASE_URL and hands it to the pg adapter", async () => {
  ...
  it("throws a clear error when the var is unset and no url is passed", async () => {
    ...
    const { getPrisma } = await import(CLIENT);
    expect(() => getPrisma()).toThrow(/SHOPIFY_READ_DATABASE_URL/);
  });
