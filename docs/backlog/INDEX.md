...
| id | item | origin | readiness | size | Workaround | src | GH |
|----|------|--------|-----------|------|----|-----|-----|
| CO1 | **Shopify member-segment pricing** (#929): auto-managed customer segment gates who redeems member/volunteer variants. NOTE: the reconciler (`lib/finance/reconcile.ts`) now flags unentitled redemption of the minted `PRG{programId}-XXXXXXXX` member-discount code (`DISCOUNT_UNAUTHORIZED`, forward + reversal passes, never auto-reverted) — an interim, detection-only stand-in for this segment-gated design, same shape as CO2's volunteer-code check | ENHANCE | IN-DESIGN | L | ? | C,D2,F | #270, #278 (open) |
