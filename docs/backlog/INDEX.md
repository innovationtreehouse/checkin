...
| PL17 | **Two timezones, one org — LIVE BUG, not future-readiness.** The org timezone is an editable setting (sysadmin, localization settings) and the server honours it: event creation converts the entered wall clock through it, and facility-trends buckets and labels against it. But the client instant formatters (`formatDate`/`formatTime`/`formatDateTime`, used across ~14 surfaces) render through a **compiled-in `America/Chicago`**. Changing the setting therefore desyncs every displayed time from every stored one, silently and with no error — an event entered as 3pm reads back at 2pm. It does not need a second region to bite; one sysadmin editing a settings page is enough. The two agree today only because the setting's default equals the constant and nobody has changed it, which is a coincidence of configuration, not a design. Violates *Principle: this codebase is not this organisation* — a particular of ours baked in rather than given. Fix: source the client formatters from the configured zone (needs a mechanism to reach the client — context provider or server-component prop), leaving the constant as seed default and offline fallback. Deliberate carve-out: `toDatetimeLocal`/`fromDatetimeLocal` stay browser-zone, the native `datetime-local` contract. **Rest of the date/time class is DONE** — calendar dates are `@db.Date`, readers UTC-pinned, age reads UTC fields, visit windows and trends bucketing fixed. Standing rule: `docs/conventions.md`, "A day is not a moment". | FIX | NEEDS-DESIGN | S–M | leave the timezone setting at its default; there is no warning if you don't | design-PR | #1346 (open) |
