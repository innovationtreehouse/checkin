...
| PL17 | **Date/time canonical-layer remediation** (from PR #1338/#1149 design review) — whole-class fix beyond P19's program-date slice: DOB written at 3 conventions (+ dedup-miss → duplicate Person, ties M18), **age-gate birthday flip that can reject an 18-yr-old (safety-adjacent, F7)**, memberSince/lastBackgroundCheck display bugs, visit-window/trends bucketing. Blocked on the calendar-date-storage DECISION. **The single-display-timezone slice is DONE**: the root layout resolves the configured `AppSettings.timezone` and installs it through `TimezoneProvider`, so `formatDate`/`formatTime`/`formatDateTime` follow the setting instead of a compiled-in zone, and `APP_TIMEZONE` is seed default + offline fallback only (`toDatetimeLocal`/`fromDatetimeLocal` stay browser-zone by design). **P19 = a slice of this. Design MERGED to main (#1338).** | FIX | IN-DESIGN | L | ? | design-PR | #1346 (open) |
