...
GH issues opened for every backlog item (2026-07-22): #1226–#1322 for the net-new set, plus pre-existing handles. Only `CM5` (external fix underway) and `RB18` (anti-item, "do NOT port") intentionally have no issue.
...
| M4 | **Archive Family** — #959 shipped a **thin first cut** (single `archivedAt` stamp + shared active-filter + write-guards + board toggle). Interview (PR #959) rework: model **TWO archive states as DB state, not text** — **Form A INVALID/DUPLICATE** (terminal, never reactivates, almost always follows a person-merge = "M18 wearing an archive costume"; NOT a purge — keep all history/audit) vs **Form B IDLE/AGED-OUT** (reactivatable on re-login). Add a **no-active-items precondition** + specific cleanup list, **state-aware self-reactivation**, and archive that **actively unsubscribes lists** / un-archive that does **NOT** auto re-add (ties SYNC-2, CM4). #959's single-state block-all guards conflict with the reactivation model | CREATE | IN-DESIGN | M | ? | B,PR#959 | #1228 (open) |
...
| M19 | **Hide archived households from the many views** that don't filter them yet (follow-on to M4/#959 soft-archive — archive blocks activity but many read surfaces still show archived families). #959's `ACTIVE_HOUSEHOLD_WHERE`/`_PERSON_WHERE` shared filter is the **reuse vehicle** for this (interview-confirmed). | ENHANCE | NEEDS-DESIGN | S | ? | owner,PR#959 | #1232 (open) |
...
| AT5 | Hour **correction + manual entry**, incl. **user self-correction of their own hours** (confirmed live gap — hours are read-only today; attendance *visits* are self-backfillable but derived/volunteer *hours* are not). Scoped (self / leader / ops), audited. **Design MERGED to main (#1352 correction surface).** | CREATE | IN-DESIGN | S | ask leader/ops/admin to correct | B,owner | #1256 (open) |
...
| id | item | origin | readiness | size | Workaround | src | GH |
|----|------|--------|-----------|------|----|-----|-----|
| SA1 | 🔴 **BG clearance stamps every household lead, not the person checked — CRITICAL SAFETY DEFECT** — `clearBackgroundCheck` household branch (`review.ts:305`) blanket-`updateMany`s the BG date onto EVERY lead, so clearing ONE adult makes an **un-checked adult read as background-checked** (one person's check satisfies another's; an uncleared adult supervising youth shows as cleared). **NOT moot** — the SA2 re-import fixed old data, but the blanket stamp never stopped, so new pollution accrues on every clearance. Active design: `checkin-app/docs/designs/1260_BG_PER_ADULT_SUBJECT.md`. Unblocks SA3 (needs a per-adult predicate) | FIX | IN-DESIGN | M | none — safety-critical | B,D2,design | #1260 (open) |
...
| SA11 | **Volunteer-onset background-check trigger (household-scoped)** (rules-fold) — an adult who starts volunteering needs a BG check AT THAT POINT, not only at intake/renewal or the annual sweep; specifically, when a SECOND adult in a household volunteers, BOTH adults need a check. Event-driven (fires on volunteer assignment), off-cycle. SA3's sweep is a periodic backstop; SA1 holds the trigger machinery — neither names this trigger or the both-adults rule. | CREATE | NEEDS-DESIGN | M | no off-cycle trigger; caught late (or missed) by the annual sweep | rules-fold | #1429 (open) |
...
**Obsolete (2026-07-21):** SA2 wipe-polluted-blanket-BG-**DATA** — no *pre-import* polluted rows remain (everyone re-imported per-adult after the DB change). **⚠ CORRECTION (2026-08-01):** the earlier speculation that this *also moots the blanket-stamp→per-adult CODE defect* was **WRONG** (verified vs main in `1260_BG_PER_ADULT_SUBJECT.md`). The `review.ts:305` blanket `updateMany` never stopped — new pollution accrues on **every** household clearance. That is a **LIVE critical safety defect** and is now **SA1 / #1260** (not dropped, not moot).

**SA1 "BG automation remainder" DECOMPOSED (2026-08-01; slice statuses CORRECTED per `1260_BG_PER_ADULT_SUBJECT.md`, verified vs main):** **cron cohort-open** = BUILT (Trigger A, `cron/person-bg-annual`) · **consent link/email** = **NOT built** (board-manual submit only, no in-app orchestration; #961 territory) · **supplier affirmation** = **meaning UNKNOWN**, needs clarification (see QUESTIONS) · **blanket-stamp→per-adult** = 🔴 **NOT built — LIVE CRITICAL SAFETY DEFECT → SA1/#1260** (my earlier "MOOT" was WRONG) · **enforcement/grace** = **SA18** (board-gated by SA5). Plus **SA17** self-attest split. Escalation half = **SA4**.
