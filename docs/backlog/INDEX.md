...
| PL17 | **Single display-timezone source** — the last open piece of the date/time class. The client instant formatters (`formatDate`/`formatTime`/`formatDateTime`) render through a compiled-in `APP_TIMEZONE`, while the server already resolves the org's configured zone from settings. The two agree only while the org runs in that one zone, and diverge silently the moment a second region deploys. Source the client formatters from the configured zone and leave the constant as seed default and offline fallback; needs a mechanism to reach the client (context provider or server-component prop). Deliberate carve-out: `toDatetimeLocal`/`fromDatetimeLocal` stay browser-zone — that is the native `datetime-local` contract, not a miss. **The rest of the class is DONE**: calendar-date storage is `@db.Date`, the readers are UTC-pinned, age reads UTC fields, the visit windows and trends bucketing are fixed. Standing rule: `docs/conventions.md`, "A day is not a moment". | FIX | NEEDS-DESIGN | S–M | single region, so the two zones agree today | design-PR | #1346 (open) |
