...
| CM2 | **Coverage-drop detection** (RESCOPED per SYNC-4 — NOT auto re-add): household loses its last synced lead → notification + board dashboard surface. Warn-only, matching the app's posture elsewhere; automation reconsidered after frequency/reasons learned | CREATE | IN-DESIGN | M | ? | B,PR#1197 | none (dep CM1) |
| CM3 | Ghost / externally-added-unknown detection — shape decided (PR #1197): same `listMembers` call, reverse diff → surfaced on a screen (board / program-lead viewpoint); sync **never auto-removes anyone it didn't add**; bulk-remove = checkbox-select rows (incl. select-all), not "remove all ghosts" | CREATE | IN-DESIGN | M | ? | B,PR#1197 | none (dep CM1) |
| CM4 | Cascading list cleanup on program/membership removal — **partially absorbed by CM1's engine** (boundary removals fall out of the desired-state diff); net-new bit = the **DENIED-newsletter exception** (SYNC-3: DENIED is the one removal from the otherwise-never-removed newsletter) | CREATE | IN-DESIGN | S | ? | B,PR#1197 | none (dep CM1, M5) |
