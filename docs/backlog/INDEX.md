...
| id | item | origin | readiness | size | Workaround | src | GH |
|----|------|--------|-----------|------|----|-----|-----|
| SA1 | **BG automation remainder**: cron cohort-open, consent link/email, supplier affirmation, blanket-stamp→per-adult migration, enforcement/grace blocking (Phase 3 consent+queue shipped; #961 shipped the escalating student-nudge slice). **BLOCKER (interview PR #961):** the nudge's `/membership` self-attest (#875) keys off the **household** process — **widening it to accept a bare PERSON_BG obligation is required, NOT deferred** (silence-on-attest is core customer service — "you did your part → reminders stop" can't land otherwise) | ENHANCE | NEEDS-DESIGN | L | ? | B,D2,PR#961 | #1260 (open) |
...
| RB20 | Vocab rename ledger (UNFINISHED.md): leadMentor→ProgramLeader, staff→Treehouse Volunteer, dues→membership-fee, ToolLevel explicit rank, dependent retire+BUG-2, payment "certified"→"manual", etc. | CHORE | READY-FOR-DEV | M | ? | C | #1322 (open) |
