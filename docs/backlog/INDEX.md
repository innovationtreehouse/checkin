...
| id | item | origin | readiness | size | Workaround | src | GH |
|----|------|--------|-----------|------|----|-----|-----|
| SA1 | **BG enforcement / grace-blocking posture** (rescoped — the old "BG automation remainder" bundle is decomposed; see § Dropped) — an un-cleared PERSON_BG obligation NEVER blocks participation / check-in / renewal today (warn-only by design). The open question: whether/when it should **gate** (hard block vs a grace period vs stay warn-only), board-gated. This is the genuinely-unbuilt remainder | ENHANCE | NEEDS-DESIGN | M | warn-only; board handles manually | B,D2 | #1260 (open, rescoped) |
...
**SA1 "BG automation remainder" DECOMPOSED (2026-08-01)** — the compound bundle in #1260 was split (it was 6 things in one issue): **cron cohort-open** = BUILT (Trigger A annual sweep, `cron/person-bg-annual`) · **consent link/email** = BUILT (Phase-3 consent+queue; #961 student-nudge slice) · **supplier affirmation** = the built 2-of-N reviewer attestation (`review.ts` attest, REQUIRED_APPROVALS=2) · **blanket-stamp→per-adult migration** = MOOT (per the SA2 note above — everyone re-imported per-adult). What remains as live items: **SA1** (rescoped → enforcement/grace-blocking posture) + **SA17** (self-attest accepts a bare PERSON_BG). The escalation half is separately **SA4**.
