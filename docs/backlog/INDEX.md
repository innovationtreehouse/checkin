...
| PL17 | **Date/time canonical-layer remediation** — whole-class fix. **Storage DECIDED and SHIPPED**: every semantic calendar date is a `@db.Date` column, which collapses the 3 DOB write conventions and closes the exact-match dedup miss that produced duplicate Person rows (was tied to M18). The standing rule now lives in `docs/conventions.md` ("A day is not a moment"); the design doc it came from is retired. **Remaining, all conformance to that rule:** (a) calendar-date displays still rendered through a wall-clock zone — memberSince on the households list + detail, DOB on the broken-households and participant-merge pages, lastBackgroundCheck on the compliance page, and the attendance filter's window labels; (b) **age still computed from local date fields, so it flips on the person's own birthday — safety-adjacent: it can reject an applicant on the morning they qualify, and it gates the write that strips a DOB**; (c) visit-window filters mix a UTC parse with local day arithmetic (costs one DST hour at a window edge, not a day); (d) facility-trends bucketing and labels run in the server's zone rather than the org's; (e) instants still render through a compiled-in zone — plumb the configured org zone to the client formatters, leaving the constant as fallback only (needs a provider design; the rest is a sweep). | FIX | READY-FOR-DEV | L | ? | design-PR | #1346 (open) |
